...
int PClient::handlePacket(pikiwidb::TcpObject* obj, const char* start, int bytes) {
  ...
  auto parseRet = parser_.ParseRequest(ptr, end); // 分析请求参数
  if (parseRet == PParseResult::error) {
    ...
    auto len = processInlineCmd(ptr, bytes, params); // 获取参数列表
    if (len == 0) {
      return 0;
    }
  ...
  PSTORE.SelectDB(db_); // select db
  FeedMonitors(params);

  const PCommandInfo* info = PCommandTable::GetCommandInfo(cmd); // 从CommandTable中获取命令处理函数
  ...
  if (PREPL.GetMasterState() != PReplState_none && !IsFlagOn(ClientFlag_master) &&
    ...
    err = PCommandTable::ExecuteCmd(params, info, IsFlagOn(ClientFlag_master) ? nullptr : &reply_); // 执行命令
